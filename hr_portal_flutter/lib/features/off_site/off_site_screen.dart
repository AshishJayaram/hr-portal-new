...
class _OffSiteScreenState extends ConsumerState<OffSiteScreen> {
  ...
  Future<void> _loadOffSites() async {
    ...
    try {
      ...
      setState(() {
        _offSites.clear();
        _offSites.addAll(offSites);
      });
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to load off-site entries: $e')),
      );
    } finally {
      setState(() {
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Off-site Tracker'),
        actions: [
          IconButton(
            icon: const Icon(Icons.add),
            onPressed: () {
              _showAddOffSiteDialog();
            },
          ),
          IconButton(
            icon: const Icon(Icons.refresh),
            onPressed: _loadOffSites,
          ),
        ],
      ),
            ...
            Row(
              children: [
                ...
                IconButton(
                  ...
                  onPressed: () {
                    _showEditOffSiteDialog(offSite);
                  },
                ),
                IconButton(
                  icon: const Icon(Icons.delete, color: Colors.red),
                  onPressed: () {
                    _showDeleteConfirmation(offSite);
                  },
                ),
              ],
            ),
