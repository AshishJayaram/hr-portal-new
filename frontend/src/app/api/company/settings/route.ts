...
export async function GET(request: Request) {
  try {
    ...
    const backendApiUrl = `${backendUrl}${backendUrl.endsWith('/api') ? '' : '/api'}/company/settings${queryString ? `?${queryString}` : ''}`;
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: { message: data.error || 'Failed to fetch company settings' } },
...
export async function PATCH(request: Request) {
  try {
    ...
    const backendApiUrl = `${backendUrl}${backendUrl.endsWith('/api') ? '' : '/api'}/company/settings`;
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: { message: data.error || 'Failed to update company settings' } },
        { status: response.status }
      );
    }
