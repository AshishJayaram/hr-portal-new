...
import { 
  ...
  type BulkSelfAssessKRAItem,
  type BulkAssessmentResponse,
  type KRASummary,
  type KRASettings,
  type User
} from "@/lib/api";
...
export default function KRAPage() {
  ...
  const [activeTab, setActiveTab] = useState<'my-kras' | 'team-kras' | 'reportees-kras' | 'assessment'>('my-kras');
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs
        ...
        activeTab={activeTab}
        onTabChange={(tab) => setActiveTab(tab as 'my-kras' | 'team-kras' | 'reportees-kras' | 'assessment')}
      />
      ...
      {activeTab === 'assessment' && (
        <AssessmentSection
          selectedYear={selectedYear}
          userKRAs={userKRAs?.data}
          reporteesKRAs={reporteesKRAs?.data}
          loadingUserKRAs={loadingUserKRAs}
          loadingReporteesKRAs={loadingReporteesKRAs}
          userRole={userRole}
          userId={userId}
          user={user}
          ...
          setIsSubmittingAssessment={setIsSubmittingAssessment}
...
function AssessmentSection({
  ...
  userKRAs?: KRA[];
  reporteesKRAs?: KRA[];
  loadingUserKRAs: boolean;
  loadingReporteesKRAs: boolean;
  userRole: string;
  userId: string;
  user?: any;
  assessmentMode: 'self' | 'manager';
  setAssessmentMode: (mode: 'self' | 'manager') => void;
  assessmentData: any[];
  setAssessmentData: (data: any[]) => void;
  isSubmittingAssessment: boolean;
  setIsSubmittingAssessment: (submitting: boolean) => void;
  ...
  useEffect(() => {
    if (assessmentMode === 'self' && userKRAs && userKRAs.length > 0) {
      ...
      const managerAssessmentData = Object.values(krasByUser).map(({ user, kras }) => ({
            ...
            <Card key={userData.userId} className="p-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {userData.kras.map((kraData, kraIndex) => {
                  const originalKra = assessmentMode === 'self' 
                    ? userKRAs?.find(k => String(k.id) === kraData.kraId)
                          ...
                          <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg">
                            <div className="flex items-center space-x-3">
                              <input
                                ...
                                onChange={(e) => handleAssessmentChange(userIndex, kraIndex, 'managerFeedbackVisible', e.target.checked)}
                                className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                              />
